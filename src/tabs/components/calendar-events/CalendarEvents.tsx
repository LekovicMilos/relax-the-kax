import React from "react";
import { CalendarEvent } from "../api/calendar";
import "./CalendarEvents.css";

interface CalendarEventsProps {
  events: CalendarEvent[];
  loading?: boolean;
  error?: string;
  isAuthenticated: boolean;
  onConnect: () => void;
  onDisconnect: () => void;
}

const CalendarEvents: React.FC<CalendarEventsProps> = ({
  events,
  loading,
  error,
  isAuthenticated,
  onConnect,
  onDisconnect,
}) => {
  if (loading && !isAuthenticated) {
    return null; // Don't show anything while checking auth status
  }

  // Show connect button if not authenticated
  if (!isAuthenticated) {
    return (
      <div className="calendar-connect-container">
        <button className="calendar-connect-btn" onClick={onConnect}>
          <svg className="google-icon" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          <span>Connect Calendar</span>
        </button>
      </div>
    );
  }

  if (loading) {
    return (
      <div className="calendar-events-container">
        <div className="calendar-loading">
          <div className="calendar-spinner"></div>
          <span>Loading events...</span>
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="calendar-events-container">
        <div className="calendar-error">
          <span>⚠️ {error}</span>
        </div>
      </div>
    );
  }

  return (
    <div className="calendar-events-wrapper">
      <div className="calendar-section">
        <div className="calendar-section-header">
          <span className="calendar-section-icon">📅</span>
          <h3>Today's Events</h3>
          {events.length > 0 && (
            <span className="calendar-count">{events.length}</span>
          )}
          <button 
            className="calendar-disconnect-btn" 
            onClick={onDisconnect}
            title="Disconnect Google Calendar"
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/>
              <polyline points="16,17 21,12 16,7"/>
              <line x1="21" y1="12" x2="9" y2="12"/>
            </svg>
          </button>
        </div>

        {events.length === 0 ? (
          <div className="calendar-empty">
            <span>✨</span>
            <p>No more events today</p>
          </div>
        ) : (
          <div className="calendar-events-list">
            {events.map((event) => (
              <EventCard key={event.id} event={event} />
            ))}
          </div>
        )}
      </div>
    </div>
  );
};

// Individual event card component
const EventCard: React.FC<{ event: CalendarEvent }> = ({ event }) => {
  const formatTime = (date: Date) => {
    return date.toLocaleTimeString("en-US", {
      hour: "numeric",
      minute: "2-digit",
      hour12: true,
    });
  };

  const getTimeDisplay = () => {
    if (event.isAllDay) {
      return "All day";
    }
    return `${formatTime(event.start)} - ${formatTime(event.end)}`;
  };

  const isHappeningNow = () => {
    const now = new Date();
    return !event.isAllDay && event.start <= now && event.end > now;
  };

  const isUpcoming = () => {
    const now = new Date();
    const diffMinutes = (event.start.getTime() - now.getTime()) / (1000 * 60);
    return diffMinutes > 0 && diffMinutes <= 15;
  };

  return (
    <a
      href={event.conferenceLink || event.htmlLink}
      target="_blank"
      rel="noopener noreferrer"
      className={`calendar-event-card ${isHappeningNow() ? "event-now" : ""} ${isUpcoming() ? "event-soon" : ""}`}
    >
      <div className="calendar-event-time">
        <span className={`time-badge ${event.isAllDay ? "all-day" : ""}`}>
          {isHappeningNow() && <span className="live-dot"></span>}
          {getTimeDisplay()}
        </span>
      </div>
      <div className="calendar-event-details">
        <div className="calendar-event-summary">{event.summary}</div>
        {event.location && (
          <div className="calendar-event-location">
            <span className="location-icon">📍</span>
            {event.location}
          </div>
        )}
        {event.conferenceLink && (
          <div className="calendar-event-meeting">
            <span className="meeting-icon">🎥</span>
            Video call
          </div>
        )}
      </div>
    </a>
  );
};

export default CalendarEvents;
